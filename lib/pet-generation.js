...
/** 从 PNG 的 IHDR 块直接读宽高，不引入图像库。 */
export function readPngSize(bytes) {
  if (!Buffer.isBuffer(bytes) || bytes.length < PNG_IHDR_OFFSET + 8) return undefined
  ...
  const width = bytes.readUInt32BE(PNG_IHDR_OFFSET)
  const height = bytes.readUInt32BE(PNG_IHDR_OFFSET + 4)
...
export function decodePngBase64(value) {
  ...
  return assertPng(Buffer.from(value, 'base64'))
}

async function readBoundedBytes(response, maxBytes = MAX_IMAGE_BYTES) {
  if (!response.body || typeof response.body.getReader !== 'function') {
    const bytes = Buffer.from(await response.arrayBuffer())
    if (bytes.length > maxBytes) throw new PetGenerationError('IMAGE_TOO_LARGE', '生成的图片超过 12 MB 限制', 502)
    return bytes
  }
  const reader = response.body.getReader()
  const chunks = []
...
export async function downloadPng(value, { fetchImpl = globalThis.fetch, signal, timeoutMs = 60_000 } = {}) {
  let target
  try {
  ...
  if (target.protocol !== 'https:') {
    throw new PetGenerationError('INSECURE_IMAGE_URL', '图片服务返回了非 HTTPS 的图片地址', 502)
  }
  let response
  try {
    response = await fetchImpl(target, {
      headers: { accept: 'image/png', 'user-agent': 'dsh-agent-pet/0.2' },
      redirect: 'error',
      signal: combineSignals(signal, timeoutMs),
    })
  } catch (error) {
    if (error?.name === 'TimeoutError' || error?.name === 'AbortError') {
      throw new PetGenerationError('PROVIDER_TIMEOUT', '图片下载超时，请稍后重试', 504)
    }
    throw new PetGenerationError('IMAGE_DOWNLOAD_FAILED', '无法下载生成的图片', 502)
  }
  if (!response.ok) {
    throw new PetGenerationError('IMAGE_DOWNLOAD_FAILED', '无法下载生成的图片', 502)
  }
  return assertPng(await readBoundedBytes(response))
}
...
export async function generateOpenAiPet({
  ...
  const inlineImage = typeof first?.b64_json === 'string' && first.b64_json.trim()
  return {
    bytes: inlineImage
      ? decodePngBase64(first.b64_json)
      : await downloadPng(first?.url, { fetchImpl, signal, timeoutMs }),
    revisedPrompt: typeof first?.revised_prompt === 'string'
      ? first.revised_prompt.slice(0, 1000)
      : undefined,
  }
}

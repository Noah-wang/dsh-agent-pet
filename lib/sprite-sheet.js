...
/**
 * 每一行描述的是一段**运动过程**，不是一个静止姿势——这是动画表的关键。
 *
 * 试过写得更细（逐帧编排「frames 1-3 …, frames 4-6 …」、单独强调首尾闭合与位置锚定），
 * 生成了四轮做对比，肉眼看下来都不如这版简洁的。而且第 4 轮只改了一行文字，
 * 八行全部变差——批次方差比提示词改动的影响更大，所以别再往这里堆字了。
 */
const MOTION_BRIEFS = Object.freeze({
  idle: 'a calm breathing loop: the body rises and settles by a few pixels, ears and tail drift slightly',
  thinking: 'a pondering loop: one paw taps the chin, the head tilts a little further up and back',
  running_tool: 'a side-view run cycle: the legs pass through contact, down, passing and up positions',
  waiting_approval: 'a waiting loop facing the viewer: the head tilts side to side, ears twitch, one blink',
  done: 'a celebration loop: crouch, leap upward with both arms raised, land and settle',
  error: 'a dejected loop: the shoulders sag lower and lift slightly, the head droops and recovers a little',
  resting: 'a sleeping loop: the curled body swells and shrinks with slow breathing',
  greeting: 'a waving loop: one raised front paw swings from one side to the other and back',
})
...
export function buildSheetPrompt({ name, description, style } = {}) {
  ...
  return [
    ...
    'HOW TO READ THE GRID: each ROW is one complete looping animation, read left to right. '
      ...
      + 'the way adjacent frames of a hand-drawn animation do. The last frame of a row must flow '
      + 'back into the first frame so the loop is seamless.',
    '',
    'Rows, top to bottom:',
    listing,
    '',
    ...
    'Every frame is a complete full body drawn at EXACTLY the same scale, with the character’s head '
      + 'the same size in every cell. Center each frame in its own invisible cell with generous margin '
      + 'so no two frames touch.',

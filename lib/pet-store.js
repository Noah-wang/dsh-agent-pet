...
import { normalizeSheetLayout } from './sprite-sheet.js'
...
function validMetadata(value, dataDir) {
  ...
  return {
    ...
    sheet: normalizeSheetLayout(value.sheet, value.sheet || {}),
    source: value.source === 'imported' ? 'imported' : 'generated',
  }
}
...
export async function saveStoredPet(dataDir, { name, description, style, bytes, sheet, source }) {
  ...
  const metadata = {
    ...
    revision,
    avatarFile,
    sheet: normalizeSheetLayout(sheet, sheet || {}),
    source: source === 'imported' ? 'imported' : 'generated',
  }

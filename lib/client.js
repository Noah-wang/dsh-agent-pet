window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const previewSequence = Object.freeze([
      { state: 'idle', label: '待机呼吸' },
      { state: 'thinking', label: '歪头思考' },
      { state: 'running_tool', label: '执行工作' },
      { state: 'waiting_approval', label: '等待确认' },
      { state: 'done', label: '完成跳跃' },
      { state: 'error', label: '失败发抖' },
    ])
    ...
    /** 状态 → 格子坐标。与 host 侧 sprite-sheet.js 的 cellForState 保持一致。 */
    function cellForState(sheet, state) {
      ...
      if (sheet && sheet.kind === 'frames') {
        return { column: 0, row: index >= 0 && index < rows ? index : 0, frames: columns }
      }
     ...
     * 按像素而不是百分比定位：动画版式要一帧一帧推进 background-position-x，
     * 百分比在 steps() 下算出来的位置和格子对不齐。
     */
    function spriteStyle(sheet, state, revision, height, motion) {
      const columns = Math.max(1, (sheet && sheet.columns) || 1)
      const rows = Math.max(1, (sheet && sheet.rows) || 1)
      ...
      const cell = cellForState(sheet, state)
      // 显示框的宽高比必须等于单格宽高比，否则背景会把宠物拉变形。
      const cellWidth = Math.round(height * (cellRatio || 1))
      const style = {
        ...base,
        width: `${cellWidth}px`,
        backgroundSize: `${columns * cellWidth}px ${rows * height}px`,
        backgroundPosition: `${-cell.column * cellWidth}px ${-cell.row * height}px`,
      }

      if (cell.frames > 1 && motion !== false) {
        // 关键帧把 x 从 0 推到整条带宽，steps() 保证每一步正好落在一格上。
        // 内联的 animation 会盖掉 CSS 里那套 transform 位移动效——形象自己会动了，
        // 再叠假动效只会抖。
        style['--agent-pet-strip'] = `${-columns * cellWidth}px`
        style.animation = `agentPetFrames ${cell.frames * FRAME_DURATION_MS}ms steps(${cell.frames}) infinite`
      }
      return style
    }

    const DEFAULT_GRID = { columns: 6, rows: 8 }

    /** 把一维的「这一列/行有没有内容」压成连续段的个数，忽略碎屑。 */
    function countRuns(filled, minimum) {
      let runs = 0
      let run = 0
      for (const value of filled) {
        if (value) {
          ...
          if (run >= minimum) runs += 1
          run = 0
        }
      }
      return run >= minimum ? runs + 1 : runs
    }

    /**
     * 从图片本身反推网格：精灵表的格子之间必然有整列/整行完全透明的间隔，
     * 数一数「有内容的连续段」就是列数和行数。用户不需要知道什么是网格。
     *
     * 返回 `{ columns, rows, confident }`。图片几乎没有透明像素时无从判断
     * （多半是用户拿了带白底的图），confident 为 false，交由调用方回落。
     */
    async function detectGrid(dataUrl) {
      ...
      const columnFilled = new Array(width).fill(false)
      const rowFilled = new Array(height).fill(false)
      let opaque = 0
      ...
      // 透明像素太少 = 这张图没有透明背景，间隔无从谈起。
      if (opaque / (width * height) > 0.95) return { confident: false }

      const columns = countRuns(columnFilled, Math.max(2, Math.round(width * 0.02)))
      const rows = countRuns(rowFilled, Math.max(2, Math.round(height * 0.02)))
      const sane = columns >= 1 && columns <= 12 && rows >= 1 && rows <= 12
      return sane ? { columns, rows, confident: true } : { confident: false }
    }

    /** 用人话说明这次导入认成了什么，而不是回吐一对数字。 */
    function describeImport(grid, detected) {
      if (grid.columns === 1 && grid.rows === 1) return '已导入这张图，作为单帧形象'
      const shape = `${grid.rows} 个状态，每个状态 ${grid.columns} 帧`
      if (!detected || detected.confident !== true) {
        return `已按标准版式导入（${shape}）。图片没有透明背景时认不出格子，如果对不上请在下面手动指定`
      }
      return `已导入：识别到 ${shape}`
    }

    /** 读成 data URL（给 canvas 识别用）和裸 base64（给上传用）。 */
    function readPng(file) {
      return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
          ...
          resolve({ dataUrl, base64: dataUrl.slice(comma + 1) })
        }
    ...
    function PetStudioForm(props) {
      ...
      // 默认自动识别；只有识别错了用户才会去展开「手动指定」。
      const manualGridState = React.useState(null)
      const manualGrid = manualGridState[0]
      const setManualGrid = manualGridState[1]
      const previewingState = React.useState(false)
      const previewing = previewingState[0]
      const setPreviewing = previewingState[1]
      const previewIndexState = React.useState(0)
      const previewIndex = previewIndexState[0]
      const setPreviewIndex = previewIndexState[1]
      ...
      const previewStep = previewSequence[previewIndex] || previewSequence[0]

      React.useEffect(() => {
        if (!previewing) return undefined
        const timer = setInterval(() => {
          setPreviewIndex((index) => (index + 1) % previewSequence.length)
        }, 1350)
      ...
      async function importSheet(event) {
        ...
        setNotice('')
        try {
          const file_ = await readPng(file)
          // 用户不需要知道什么是网格：从图里的透明间隔自己数出来。
          const detected = manualGrid || await detectGrid(file_.dataUrl).catch(() => ({ confident: false }))
          const grid = detected.confident
            ? { columns: detected.columns, rows: detected.rows }
            : DEFAULT_GRID
          const kind = grid.rows === DEFAULT_GRID.rows && grid.columns > 1 ? 'frames' : undefined
          ...
          const response = await fetch('/api/agent-pet/import', {
            ...
            body: JSON.stringify({
              ...
              style,
              image: file_.base64,
              sheet: { columns: grid.columns, rows: grid.rows, ...(kind ? { kind } : {}) },
            }),
          })
          const payload = await readJson(response)
          if (!response.ok) {
          ...
          setNotice(describeImport(grid, detected))
          if (props.onUpdated) props.onUpdated(payload)
        } catch (failure) {
      ...
      React.createElement('div', { className: 'agentPetStudio__preview' },
        React.createElement('div', { className: 'agentPetStudio__live' },
          React.createElement('div', {
            className: `agentPetStudio__liveStage agentPet--${stateClass[previewStep.state] || 'idle'}`,
          }, React.createElement('div', {
            className: 'agentPet__body',
            role: 'img',
            'aria-label': `${props.petName || 'Momo'}，${previewStep.label}`,
            style: spriteStyle(props.sheet, previewStep.state, props.revision, 106, true),
          },
          ...
          previewStep.state === 'running_tool'
            ? React.createElement('span', { className: 'agentPet__tool' }, '⚙')
            : null)),
          React.createElement('div', { className: 'agentPetStudio__liveCopy' },
            React.createElement('strong', null, `动作预演：${previewStep.label}`),
            React.createElement('span', null, '先用现有姿势模拟 Agent 工作节奏，不调用生图模型。'),
            React.createElement('button', {
              type: 'button', className: 'agentPetStudio__previewButton',
              onClick: () => {
                setPreviewIndex(0)
                setPreviewing(!previewing)
              },
            }, previewing ? '■ 停止预演' : '▶ 播放动作预演'))),
        React.createElement('div', {
          ...
          React.createElement('span', {
            key: pose,
            title: pose,
            style: spriteStyle(props.sheet, pose, props.revision, 52, true),
          }))),
      ...
      React.createElement('p', { className: 'agentPetStudio__meta' },
        `绘制会生成一张逐帧动画精灵表：八个状态，每个状态六帧。调用 ${generation.model || '图片模型'}，可能产生服务商费用；每次只能生成一只。`),
      ...
      // 设置页空间充足，这条路径直接摊开；弹窗里才折叠。
      React.createElement(props.expanded ? 'section' : 'details', { className: 'agentPetStudio__external' },
        React.createElement(props.expanded ? 'h4' : 'summary', null, '用别的 AI 生成（不需要 API Key）'),
        React.createElement('p', { className: 'agentPetStudio__meta' },
        ...
        React.createElement('div', { className: 'agentPetStudio__actions' },
          ...
          React.createElement('button', {
            type: 'button', className: 'agentPetStudio__upload', disabled: locked,
        ...
        exported ? React.createElement('div', { className: 'agentPetStudio__exported' },
          ...
          React.createElement('p', { className: 'agentPetStudio__meta' },
            `状态顺序：${(exported.layout && exported.layout.poses ? exported.layout.poses : []).join(' → ')}`))
          : null,

        // 网格默认自动识别。这里只给识别错了的人一条退路，正常用户不用展开。
        React.createElement('details', { className: 'agentPetStudio__manual' },
          React.createElement('summary', null, '识别错了？手动指定'),
          React.createElement('p', { className: 'agentPetStudio__meta' },
            '导入时会从图片的透明间隔里数出有几行几列。只有那张图没有透明背景、'
            + '或者格子之间挨得太紧时才需要手动填。'),
          React.createElement('div', { className: 'agentPetStudio__gridRow' },
            React.createElement('label', null, '每个状态几帧',
              React.createElement('input', {
                type: 'number', min: 1, max: 12, disabled: locked,
                value: manualGrid ? manualGrid.columns : DEFAULT_GRID.columns,
                onChange: (event) => setManualGrid({
                  columns: Number(event.target.value) || 1,
                  rows: manualGrid ? manualGrid.rows : DEFAULT_GRID.rows,
                }),
              })),
            React.createElement('label', null, '几个状态',
              React.createElement('input', {
                type: 'number', min: 1, max: 12, disabled: locked,
                value: manualGrid ? manualGrid.rows : DEFAULT_GRID.rows,
                onChange: (event) => setManualGrid({
                  columns: manualGrid ? manualGrid.columns : DEFAULT_GRID.columns,
                  rows: Number(event.target.value) || 1,
                }),
              })),
            manualGrid
              ? React.createElement('button', {
                type: 'button', className: 'agentPetStudio__ghost', onClick: () => setManualGrid(null),
              }, '回到自动识别')
              : React.createElement('span', null, '改了就按你填的来')))))
    }
    ...
    function PetOverlay(props) {
      ...
      React.createElement('div', {
        ...
        style: spriteStyle(sheet, rawState, revision, 128, motion),
      },
        React.createElement('span', { className: 'agentPet__spark agentPet__spark--one' }, '✦'),
        React.createElement('span', { className: 'agentPet__spark agentPet__spark--two' }, '·'),
    ...
    function apply(ctx) {
      const css = `
        ...
        .agentPetStudio__live{display:flex;align-items:center;gap:14px;min-height:116px;padding:8px 10px;border-radius:10px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-1) 84%,#ece8ff)}
        .agentPetStudio__liveStage{flex:0 0 118px;min-height:112px;display:grid;place-items:center;overflow:visible}
        .agentPetStudio__liveCopy{min-width:0;display:flex;flex:1;flex-direction:column;align-items:flex-start;gap:6px}.agentPetStudio__liveCopy strong{font-size:13px;color:var(--dsw-alias-label-primary)}.agentPetStudio__liveCopy span{font-size:11px;line-height:1.45;color:var(--dsw-alias-label-secondary)}
        .agentPetStudio__previewButton{padding:7px 12px;border:0;border-radius:999px;background:#7565ed;color:#fff;font:inherit;font-size:12px;font-weight:620;cursor:pointer}.agentPetStudio__previewButton:hover{background:#6655dc}
        ...
        .agentPetStudio__external h4{margin:0;font-size:12px;font-weight:620;color:var(--dsw-alias-label-primary)}
        .agentPetStudio__external>*{margin-top:8px}
        .agentPetStudio__upload{padding:8px 16px;border:1px dashed #7565ed;border-radius:999px;background:color-mix(in srgb,var(--dsw-alias-bg-layer-1) 88%,#8d7cff);color:#5a49c9;font:inherit;font-size:13px;font-weight:620;cursor:pointer}
        .agentPetStudio__upload:hover{background:color-mix(in srgb,var(--dsw-alias-bg-layer-1) 78%,#8d7cff)}
        .agentPetStudio__upload:disabled{opacity:.55;cursor:not-allowed}
        .agentPetStudio__manual{margin-top:10px;border-top:1px solid var(--dsw-alias-border-l1);padding-top:8px}
        .agentPetStudio__manual summary{cursor:pointer;font-size:11px;color:var(--dsw-alias-label-secondary)}
        .agentPetStudio__manual>*{margin-top:8px}
        .agentPetStudio__gridRow{display:flex;align-items:center;gap:10px;flex-wrap:wrap;font-size:12px;color:var(--dsw-alias-label-secondary)}
        ...
        @keyframes agentPetFrames{to{background-position-x:var(--agent-pet-strip)}}

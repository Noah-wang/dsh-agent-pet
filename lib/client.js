window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PetStudio(props) {
      ...
      React.createElement('div', { className: 'agentPetStudio__preview' },
        React.createElement('img', {
          src: `/api/agent-pet/avatar?revision=${encodeURIComponent(props.revision || 'default')}`,
          alt: `当前宠物 ${props.petName || ''}`,
          draggable: false,
        }),
        React.createElement('span', null, busy || (generation.busy ? '正在绘制…' : `当前：${props.petName || 'Momo'}`))),
      ...
      React.createElement('p', { className: 'agentPetStudio__meta' },
        `绘制会调用 ${generation.model || '图片模型'}，可能产生服务商费用；每次只能生成一只。`),
    ...
    function PetOverlay(props) {
      ...
      React.createElement('div', { className: 'agentPet__body' },
        ...
        React.createElement('img', {
          src: `/api/agent-pet/avatar?revision=${encodeURIComponent(revision)}`,
          alt: `${name}，${label}`,
          draggable: false,
        }),
    ...
    function apply(ctx) {
      const css = `
        ...
        .agentPetStudio__preview{display:flex;align-items:center;gap:12px;padding:10px;border:1px dashed var(--dsw-alias-border-l1);border-radius:12px}.agentPetStudio__preview img{width:76px;height:68px;object-fit:contain}.agentPetStudio__preview span{font-size:12px;color:var(--dsw-alias-label-secondary)}

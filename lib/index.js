...
export function apply(ctx, config = {}) {
  ...
  async function importRoute(request) {
    ...
    try {
      ...
      if (sheet.columns * sheet.rows > 1 && size) {
        // 网格切分依赖整除，尺寸对不上会让每格都偏移。
        if (size.width % sheet.columns || size.height % sheet.rows) {
          return jsonResponse(400, {
            error: `图片 ${size.width}×${size.height} 不能被 ${sheet.columns}×${sheet.rows} 整除，请裁剪后再导入`,
            code: 'SHEET_NOT_DIVISIBLE',
          })
        }
      }
